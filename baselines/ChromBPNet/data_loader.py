...
class SingleTaskDataset:
    ...
    def to_tf_dataset(self, batch_size, shuffle=False):
        """将数据转换为 tf.data.Dataset"""
        # 重塑并转置序列：(n_samples, 20000) -> (n_samples, 5000, 4) -> (n_samples, 4, 5000)
        seq1 = tf.reshape(self.seq1_data, [-1, 5000, 4])
        seq1 = tf.transpose(seq1, [0, 2, 1])  # (n_samples, 4, 5000)
        seq2 = tf.reshape(self.seq2_data, [-1, 5000, 4])
        seq2 = tf.transpose(seq2, [0, 2, 1])  # (n_samples, 4, 5000)
        labels = tf.expand_dims(self.labels, axis=-1)  # (n_samples, 1)
        
        # 创建 tf.data.Dataset
        dataset = tf.data.Dataset.from_tensor_slices(
            ({"seq1": seq1, "seq2": seq2}, {"epi_output": labels, "eei_output": labels})
        )
...
def create_multitask_data_loaders(
    ...
    for task_name, paths in tasks.items():
        ...
        task_loaders[task_name] = {
            'train': train_dataset.to_tf_dataset(batch_size, shuffle=True),
            'val': val_dataset.to_tf_dataset(batch_size, shuffle=False),
            'test': test_dataset.to_tf_dataset(batch_size, shuffle=False)
        }
        ...
        for split_name, loader in loaders.items():
            ...
            try:
                for batch_idx, batch in enumerate(loader):
                    seq1 = batch[0]['seq1']
                    seq2 = batch[0]['seq2']
                    labels = batch[1]['epi_output']  # 或 eei_output
                    print(f"    Batch {batch_idx}:")
                    print(f"      seq1 shape: {seq1.shape}")  # 应该是 (batch_size, 4, 5000)
                    print(f"      seq2 shape: {seq2.shape}")  # 应该是 (batch_size, 4, 5000)
                    print(f"      labels shape: {labels.shape}")
                    print(f"      seq1 数据范围: [{tf.reduce_min(seq1):.3f}, {tf.reduce_max(seq1):.3f}]")
                    print(f"      seq2 数据范围: [{tf.reduce_min(seq2):.3f}, {tf.reduce_max(seq2):.3f}]")
