...
class PositionEncoding(Layer):
    ...
    def call(self, inputs):
        batch_size = tf.shape(inputs)[0]  # 动态获取批量大小
        ...
        # 增加批量维度并广播
        position_encodings = tf.expand_dims(position_encodings, 0)  # [1, seq_length, model_dim]
        position_encodings = tf.tile(position_encodings, [batch_size, 1, 1])  # [batch_size, seq_length, model_dim]
        return position_encodings
...
#         if position_encodings.shape[-1] != embeddings.shape[-1]:
#             if position_encodings.shape[-1] < embeddings.shape[-1]:  
#                 position_encodings = tf.pad(position_encodings, [[0, 0], [0, 0], [0, embeddings.shape[-1] - position_encodings.shape[-1]]])
#             else:
#                 position_encodings = position_encodings[..., :embeddings.shape[-1]]
...
#             if attention_out.shape[-1] != encodings.shape[-1]:
#                 if attention_out.shape[-1] < encodings.shape[-1]:
#                     attention_out = tf.pad(attention_out, [[0, 0], [0, 0], [0, encodings.shape[-1] - attention_out.shape[-1]]])
#                 else:
#                     attention_out = attention_out[..., :encodings.shape[-1]]
#             attention_out += encodings
#             attention_out = LayerNormalization()(attention_out)
#             ff_out = self.feedforward(attention_out)
#             if ff_out.shape[-1] != attention_out.shape[-1]:
#                 if ff_out.shape[-1] < attention_out.shape[-1]:
#                     ff_out = tf.pad(ff_out, [[0, 0], [0, 0], [0, attention_out.shape[-1] - ff_out.shape[-1]]])
#                 else:
#                     ff_out = ff_out[..., :attention_out.shape[-1]]
